...
from lib.utils.torch import pad_tensor_list
from lib.utils.thing import thing2dev
...
class ObjectTensors(nn.Module):
    def __init__(self, device):
        ...
        self.obj_tensors = thing2dev(construct_obj_tensors(OBJECTS), device)
        self.dev = None
...
def construct_obj(object_model_p):
    ...
    top_sub_p = f"/data/arctic/downloads/data/meta/object_vtemplates/{obj_name}/top_keypoints_300.json"
    ...
    with open("/data/arctic/downloads/data/meta/object_meta.json", "r") as f:
...
def construct_obj_tensors(object_names):
    ...
    for k in object_names:
        object_model_p = f"/data/arctic/downloads/data/meta/object_vtemplates/%s" % (k)
